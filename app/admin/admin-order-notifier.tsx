...
export default function AdminOrderNotifier({ enabled }: { enabled: boolean }) {
  ...
  return (
    <>
      <button className={`fixed right-4 top-4 z-50 grid h-10 w-10 place-items-center rounded-full border bg-white text-brand shadow-sm hover:bg-brand hover:text-white ${enabled && armed && connected ? "ring-2 ring-emerald-400" : ""}`} title={enabled ? `New-order notifications ${connected ? "connected" : "connecting"}` : "Notifications disabled in Settings"} onClick={armNotifications}>
